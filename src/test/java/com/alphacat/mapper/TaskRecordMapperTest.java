...
public class TaskRecordMapperTest {
    ...
    public void A_add() {
        ...
        taskRecordMapper.add(new TaskRecord(1,1,"1234567",5));
        taskRecordMapper.add(new TaskRecord(1,3,"7654321",3));
    }

    @Test
    public void B_getByWorker() {
        Assert.assertEquals(2,taskRecordMapper.getByWorker(1).size());
        Assert.assertEquals(1,taskRecordMapper.getByWorker(1).get(0).getTaskId());
        Assert.assertEquals("7654321",taskRecordMapper.getByWorker(1).get(1).getPicOrder());
    }

    @Test
    public void C_get() {
        Assert.assertTrue("1234567".equals(taskRecordMapper.get(1,1).getPicOrder()));
    }

    @Test
    public void D_incPicDoneNum() {
        taskRecordMapper.incPicDoneNum(1,1);
        Assert.assertEquals(6, taskRecordMapper.get(1, 1).getPicDoneNum());
    }

    @Test
    public void E_decPicDoneNum() {
        taskRecordMapper.decPicDoneNum(1,1);
        Assert.assertEquals(5, taskRecordMapper.get(1, 1).getPicDoneNum());

    }

    @Test
    public void F_getTaskIds() {
        Assert.assertEquals(1, taskRecordMapper.getTaskIds(1)[0]);
        Assert.assertEquals(3, taskRecordMapper.getTaskIds(1)[1]);
    }

    @Test
    public void G_getWorkerNum(){
        Assert.assertEquals(1, taskRecordMapper.getWorkerNum(1).intValue());
    }

    @Test
    public void H_getTagSum(){
        Assert.assertEquals(5, taskRecordMapper.getTagSum(1).intValue());
    }
